...
#glob_path = Path(post_dir)
filenames = [str(pp) for pp in glob.glob(os.path.join(post_dir, "**", "*.md"), recursive=True)]
#glob_path = Path(draft_dir)
filenames += [str(pp) for pp in glob.glob(os.path.join(draft_dir, "**", "*.md"), recursive=True)]


""" def check_os():
    if platform.system() == 'Windows':
        glo
        filenames = os.listdir(post_dir + "*.md")
